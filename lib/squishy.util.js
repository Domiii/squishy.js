...
squishy.objToStringMaxDepth = 20;
...
squishy.objToString = function(obj, json, maxDepth, layer, indent) {
    ...
    if (layer > squishy.objToStringMaxDepth)  {
        //throw new Error("Possible cyclic object nesting in squishy.objToString().");
        console.warn("Possible cyclic object nesting in squishy.objToString().");
        return '...';
    }
    ...
    if (obj == null) {
        ...
        var iterator = function(propName) {
            ...
            var propStr = squishy.objToString(prop, json, maxDepth, layer+1, indent);
...
squishy.extendClass = function(superClass, extendedCtor, extendedClassMembers, extendedStaticMembers) {
    console.assert(superClass, 'Invalid call to `extendClass`, first argument is missing: `superClass`');
